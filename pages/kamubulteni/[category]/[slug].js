...
export async function getStaticPaths() {
	const contents = await fetch(`${process.env.NEXT_PUBLIC_API}/kamubulteni`);
	const json = await contents.json();
	const urls = await json.map(content => {
		return {
			params: {
				category: content.url.split("/")[0],
				slug: content.url.split("/")[1],
			},
		};
	});
...
export async function getStaticProps(context) {
	...
	const response = await fetch(
		`${process.env.NEXT_PUBLIC_API}/kamubulteni/${params.category}/${params.slug}`,
		{
			method: "GET",
			headers: {
